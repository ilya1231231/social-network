  ...
  <div class="row">
    ...
    <div class="col-lg-4 col-lg-offset-3">
      ...
      @elseif ( Auth::user()->hasFriendRequestReceived($user) )
        <a href="{{ route('friends.accept', ['username'=> $user->username] )}}" class="btn btn-success">Подтвердить</a>
      <!-- Если он в друзьях,то будет при поиске отображаться это -->
      @elseif ( Auth::user()->isFriendWith($user))
        {{ $user->getFirstNameOrusername()}} у вас в друзьях.
        <!-- Если id аутентифициорванного пользователя не совпадает с юзером,которого нужно добавить
        (чтобы не отображалось в профиле "Добавить в друзья") -->
      @elseif( Auth::user()->id !== $user->id)
        <a href="{{ route('friends.add', ['username'=> $user->username]) }}"
            class="btn btn-success">Добавить в друзья</a>
      @endif
